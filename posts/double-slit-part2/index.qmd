...
categories: [Advanced, quantum mechanics, Schrödinger equation, Gaussian integrals, python]
toc: true
...
In any graduate textbook on quantum theory, you'll find a solution to the Schrödinger equation that is a wave "packet."
This is a wavefunction $\Psi(x, t)$ that has the form of a Gaussian ($\propto e^{\alpha x^2}$). In other words, the wavefunction 
is slightly spread out over space rather than all the probability being concentrated at a point. You can get some cool 
intuition and solve more problems with this type of wavefunction. Aside: it's also easy to demonstrate the relationship
between position uncertainty and momentum uncertainty with such a wave packet. 

**Question for the reader**:
_How would you show that relationship using a wave packet? (Hint: the momentum space wavefunction is essentially 
obtained through a 
Fourier transform of the [Gaussian] position wavefunction, and describes how spread out the momentum is - with
more spread out being higher uncertainty)_.

That this solves the Schrödinger equation can be easily shown. You can find the derivation in most graduate texts, 
including @cohen-tannoudji.

#### Quick detour on the Feynman approach

I recently dusted off Feynman and Hibbs' Quantum Mechanics and Path Integrals [@feynman-hibbs], a fascinating treatment of
quantum mechanics that doesn't use much of the linear algebra and operator-based formulation. The core idea
here (without regurgitating everything) is that there is a cool, different way of calculating the probability
amplitude of a particle that starts at $(x_0, t_0)$ in spacetime arriving at $(x_1, t_1)$.

The propagator for a free particle (that which is traveling through space with no potential anywhere)
is defined as follows (presented without justification; see Feynman and Hibbs):

$$
K(x_b, t_b; x_a, t_a) = \sqrt{\frac{m}{2\pi i \hbar (t_b - t_a)}}\;
\exp\!\left[\frac{i m (x_b - x_a)^2}{2\hbar (t_b - t_a)}\right]
$$

This happens to be exactly what we're looking for - the propagator tells us the probability of arriving at the top
or bottom slit at a given time, and then due to its linearity can be multiplied and integrated with the propagator
for the particle starting at either slit and arriving at some point on the detector screen.

So without further ado, let's jump into the math.

#### The setup

Now we have a solution that models a particle in real space, slightly spread out, and which solves the
Schrödinger equation. Let's make all this concrete with our experiment. 
...
A Gaussian packet times a Gaussian slit is still Gaussian, and free evolution using the propagator
maps Gaussians to Gaussians, so every step stays analytic and no grid is needed.

Throughout, $y$ is the transverse coordinate and the along-axis drift is just the time axis.
The packet reaches the slits at $t_1 = L_1/v$ and the screen at
$t_2 = L_2/v$, with $v = \hbar k_0/m$. The incident state is the standard free
Gaussian wave packet of Cohen-Tannoudji [-@cohen-tannoudji, Complement $G_I$].

##### The incident packet

Starting with a Gaussian wave packet along $y$,
...
Applying the propagator, we can convolve @eq-psi0 with the propagator over $t_1$, writing
...
$C = sy^2$. A complex Gaussian integral identity, together with $1/s = -2i\hbar t_1/m$,
...
$\Delta y(t_1) = \sigma_0\sqrt{1 + (\hbar t_1/2m\sigma_0^2)^2}$.

##### The Gaussian slits

For the sake of reaching a closed form solution, I've modeled the two openings as Gaussian apertures centered at
$y_\pm = \pm y_s$, with
...
Again, we convolve $\phi$ with the free kernel - this time over $t_2$. Then we write
...
Since $\lambda_2$ is imaginary, $\operatorname{Re} p = \operatorname{Re}(1/4\alpha_1) + 1/2\beta^2 > 0$
...
The $\cos(2\kappa_i y_D)$ term is the interference fringes that has that classic quantum signature.
...
#| fig-cap: "Closed-form Gaussian-slit intensity on the detection screen."
#| code-fold: true
import numpy as np
import matplotlib.pyplot as plt
import dslit
...
psi_p, psi_m = dslit.psi_per_slit(yD)
I = dslit.screen_intensity(psi_p, psi_m)
I = I / I.max()

fig, ax = plt.subplots(figsize=(8, 4))
ax.plot(yD * 1e6, I, color="#185FA5", lw=1.5)
ax.set_ylabel("normalized intensity")
ax.set_title("Gaussian-slit fringes")
plt.show()
```
